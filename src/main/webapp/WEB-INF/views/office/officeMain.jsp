		...
		$.ajax({
			...
			success : function(result){
				...
				for(let i =0; i<result.length; i++){
					...
					} else if(r.patWait == '검사중'||r.patWait =='검사완료'||r.patWait =='치료대기'||r.patWait =='치료중'||r.patWait =='처방처치중'||r.patWait =='검사대기'||r.patWait =='치료완료'||r.patWait =='처치중') {
						c5 += 1;
						txt5 += `
				        	<table style="width : 100%; text-align : left; border-bottom : 1px solid black; margin-bottom : 15px;">
				    			<tr>
				    			...
				    			<tr>
				    				<td style="width : 65%;"><span style="font-size : 0.7rem;">\${r.patBrthdy} | \${r.patGen} | \${r.patFirst}<span></td>
				    				<td><button class="btn btn-xs btn-secondary" "disabled" style="font-size : 0.7rem;">\${r.patWait}</button></td>
				    			</tr>
			   				</table>
						`;
        ...
        if(xhr.readyState == 4 && xhr.status == 200){
			...
			for(let i=0; i<rslt.length; i++){
	            ...
	            txt +=`<table style="width: 95%; margin : 5px; border-bottom : 1px solid gray;">
         					...
         					<tr>
         						<td>\${yN}</td>
         						<td><button onclick="tClick(\${rslt[i].patCode},\${rslt[i].treatNo},'\${rslt[i].receptTime}')" class="btn btn-sm btn-primary">상세보기</button></td>
         					</tr>
         				</table>`;
			}
			dateTable.html(txt);
